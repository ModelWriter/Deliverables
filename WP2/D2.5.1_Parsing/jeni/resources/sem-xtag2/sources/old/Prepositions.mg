...
{ 
	<syn>{ 
		node Xr(name=VP_r)[bot=[idx=?Idx]]; node Xf(name=VP_f)[top=[idx=?Idx]];
...
{ 
	{<syn>{ 
  		node Xr(name=VP_r)[bot=[idx=?E]]; node Xf(name=VP_f)[top=[idx=?E]]; node Xs(name=NP_s)[top=[idx=?P]];
  		node X1(name=PP1); node Xa(name=P_a);
  		Cf=vp; C1=pp; Ca=prep; Cs=np
  	};
  	PREP-rfVP[]; PREP-rfV-conj[]; PREP-ax1-assign-case-wh[]; PREP-x1s-assign-case[]}*=[lemma=?L];
  	{<sem>{relation(?L, ?E, ?P)}} 
}
...
{ 
	{<syn>{
        	node Xr(name=NP_r)[bot=[idx=?P1]]; node Xf(name=NP_f)[top=[idx=?P1]]; node Xs(name=NP_s)[top=[idx=?P2]];
        	node X1(name=PP1);node Xa(name=P_a);
 		Cf=np; C1=pp; Ca=prep; Cs=np
 	};
 	PREP-rfN[]; PREP-rfN-gerund[]; PREP-ax-assign-case[]; PREP-sx-assign-case[]}*=[lemma=?L];
	{<sem>{relation(?L, ?P1, ?P2)}}
}
