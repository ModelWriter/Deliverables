...
public class OntoModel {
	...
	private int countNewAxiomsDeclIncl;		// count all new axioms including declarations of classes and properties
		...
		for (String classNamewithCase:generateCaseCombinationsSimple(className)) {
			for (String nameSpace:nameSpaces.values()) {
				...
				if (ontology.containsClassInSignature(fetchIRI)) {
					...
					countExistingConcepts += 1;  // for a current axiom, take into account current state of the ontology
		...
		for (String relationNamewithCase:generateCaseCombinationsSimple(relationName)) {
			for (String nameSpace:nameSpaces.values()) {
				...
				if (ontology.containsDataPropertyInSignature(fetchIRI)) {
					...
					countExistingRelations += 1;  // for a current axiom, take into account current state of the ontology
			...
			for(int i = 0; i < mweParts.length; i++) {
				...
				if (!mweParts[i].isEmpty()) {
					if (i != mweParts.length - 1) {
						mweUpper.append(mweParts[i].substring(0, 1).toUpperCase() + mweParts[i].substring(1) + " ");  //Sas Ramp Boom
					}
					else {
						mweUpper.append(mweParts[i].substring(0, 1).toUpperCase() + mweParts[i].substring(1)); //don't append space
					}
				}
			}
	...
	private OWLClass addNewClass(String className) {
		IRI classIRI = IRI.create(nameSpaces.get("xml:base")+className); // The new Concept to be made will always be made using the base uri
		...
		/*IRI individIRI = IRI.create(nameSpaces.get("xml:base") + className + "1");
		OWLIndividual newIndivid = dataFactory.getOWLNamedIndividual(individIRI);
		addClassAssertionAxiom(newClass, newIndivid);*/
		return newClass;
	}
	
	
	private OWLObjectProperty addNewObjectProperty(String relationName) {
		IRI relationIRI = IRI.create(nameSpaces.get("xml:base")+relationName); // The new relation to be made will always be made using the base uri
	...
	private OWLAxiom getAxiomFromString(String axiomString) {
	    ...
	    if (axiomString.startsWith("SubClassOf")||axiomString.startsWith("EquivalentClasses")) {
	    	try {  // parse only SubClassOf and EquivalentClasses axioms!	
	    		...
	    		System.out.println("The axiom doesn't follow the functional owl syntax: " + axiomString + e.getMessage());
	    		log.log(Level.SEVERE, "The axiom doesn't follow the functional owl syntax: " + axiomString + e.getMessage());
	...
	private void addAxiom(OWLAxiom axiom) {
		...
		log.log(Level.FINER, "The axiom was added. Count of axioms in the enriched ontology: " + ontology.getAxiomCount());
	}
	...
	private void addObjectPropertyAssertionAxiom(OWLIndividual subject, OWLObjectProperty predicate, OWLIndividual object) {
	    ...
	    for (String conceptName : conceptNames) {
	    	...
	    	if (cls == null){
	    		...
	    		if (clsLemma != null) {
	    			...
	    			// we don't find any existing class, so search in rdfs labels and create hypothetical matchings
		    		...
		    		if (conceptLemmatised.contains("_+_")) {
			    		...
			    		String conceptNameMweUri = "<http://airbus-group.installsys/component#" + conceptNameMWE + ">";
			    		entitiesToUri.put(conceptNameNoColon, conceptNameMweUri);
			    		log.log(Level.FINER, "add a new class (lemmatised) to the ontology: <http://airbus-group.installsys/component#" + conceptNameMWE);
			    		newConceptsInAxiom.add(conceptNameMWE);
		    		}
		    		else {
		    			addNewClass(conceptLemmatised);
		    			String conceptLemmatisedUri = "<http://airbus-group.installsys/component#" + conceptLemmatised + ">";
			    		entitiesToUri.put(conceptNameNoColon, conceptLemmatisedUri);
			    		log.log(Level.FINER, "add a new class (lemmatised) to the ontology: <http://airbus-group.installsys/component#" + conceptLemmatised + ">");
	...
	private Object[] addRelations(Set<String> relationNames) {
	    ...
	    for (String relationName : relationNames) {
	    	...
	    	if (property == null){
	    		...
	    		entitiesToUri.put(relationNameNoColon, "<http://airbus-group.installsys/component#" + relationNameNoMwe + ">");
	    		log.log(Level.FINER, "add a new property to the ontology: <http://airbus-group.installsys/component#" + relationNameNoMwe + ">");
		 	...
		 	if(consistency) {
			    message.add("The ontology is consistent, and it has no unsatisfiable classes.");
		 		...
		 		if (!consistencyDoubleCheck) {
		 			System.out.println("Warning! The removed axiom was not removed properly, and the inconsistency/unsatisfiability issue is still present!");
		 			log.log(Level.FINER, "Warning! The removed axiom was not removed properly, and the inconsistency/unsatisfiability issue is still present!");
		 		}
		 	}
	    }
	...
	public void writeOutputToFiles(String outputFolderName) throws UnsupportedEncodingException, FileNotFoundException, IOException {
		// write output of ConceptSearch class: matching to RDFS-labels, partial matching of RDFS-labels and hypothetical superclasses
		ConceptSearch cs = ConceptSearch.getInstance();
		...
		File existingRelationsFile = new File(outputFolderName+"ontology_enrichment/existing_relations.txt");
		...
		File rejectedInconsistentAxiomsFile = new File(outputFolderName+"ontology_enrichment/rejected_inconsistent_axioms.txt");
		File statsFile = new File(outputFolderName+"ontology_enrichment/stats.txt");
		StringBuilder str = new StringBuilder();
		str.append("Numbers of new classes/relations are given without repetition and in comparison to the initial ontology.\n\n");
		...
		str.append("\tNb. of rejected SIDP axioms due to their presence in the initial ontology = " + countAxiomsRejectedRedundInitOnt + "\n");
		str.append("\tNb. of rejected SIDP axioms that caused the inconsistency/unsatisfiability = " + countAxiomsRejectedInconsistent + "\n\n");
		str.append("====Summary====\n");
		str.append("The initial ontology contains " + initialCountAxioms + " axioms.\n");
		str.append(countNewAxiomsDeclIncl + " axioms were added (including declaration axioms, excluding individuals).\n");
		str.append("The enriched ontology contains " + ontology.getAxiomCount() + " axioms in total (with individuals).\n");
	...
	public boolean checkOntologyConsistency(OWLOntology ontology, Object[] concepts) {
		...
		/*if (unsatClasses.getSize() > 1) {
			...
			log.log(Level.WARNING, "Warning! The ontology has unsatisfiable classes: " + unsatClasses);
		}
		else {
