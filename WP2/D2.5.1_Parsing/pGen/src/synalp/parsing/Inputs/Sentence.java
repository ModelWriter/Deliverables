...
public class Sentence {
	...
	static {
		// Currently, the trees in our grammar don't support determiner. But these things should also be in the adjoiningWordsToken list ??
		/*
		skipWords.add("a");
		skipWords.add("the");
		*/
	...
	public Sentence (String text) {
		...
		for (int i=0;i<divs.length; i++) {
			...
			words.add(word);
			if (!quantifiers.contains(token)&&!PoS.conjunctions.contains(token)&&!PoS.prepositions.contains(token)&&!skipWords.contains(token)) {
	...
	private void computePossibleCoAnchors() {
		...
		// The adjoiningWordsToken are also not present in the lexical items as coanchors. They are present as anchors in adjoining trees.
		for (Word w:words) {
