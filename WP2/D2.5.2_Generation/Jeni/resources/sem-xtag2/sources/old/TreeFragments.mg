...
declare ?Idx ?L
{
	<syn>{ node ?V(mark=anchor,name=V_a)[cat=v, top=[pass = -, idx=?Idx]]}*=[lemma=?L];
	<sem>{event(?Idx); evttype(?Idx,?L)}
}
...
declare ?Idx ?L
{
	<syn>{node ?V(mark=anchor,name=V_a)[cat=v, top =[pass = +, mode = ppart, idx=?Idx]]}*=[lemma=?L];
	<sem>{event(?Idx); evttype(?Idx, ?L)}       
}
...
declare ?SubjIdx
{
        <syn> { node ?Subj(mark=subst,name=NP_subj)[cat=np,bot=[idx=?SubjIdx]] }; ?fAC = nom   
}
...
declare ?ObjIdx
{<syn> {
        node ?compl(color=red,mark=subst,gf=obj,name=NP_obj,rank=2)[cat=np,bot=[xp = +, idx=?ObjIdx]]
...
declare ?SubjIdx
{
	<syn>{ node ?compl (rank=7)[bot=[idx=?SubjIdx]]; node ?P (mark=flex)[cat=by] }
}
...
import CanComplement[]
export ?ComplIdx
declare ?Ve ?xNP ?ComplIdx
{<syn> { 
         node ?compl (color=red)[cat=vp,bot=[compar= -]]{
              node ?Ve(color=red)[cat=v,phon=e]
              node ?xNP(color=red,mark=subst)[cat=np,gerund= -, case=@{nom,acc}, top=[idx=?ComplIdx]]
                                                          }
        }
}


%%% without semantics done
...
import RelativeOvertComplement[]
       VerbCaseAssigner[] 
{<syn> { 
         node ?Trace(rank=2,gf=obj,name=NP_obj)
