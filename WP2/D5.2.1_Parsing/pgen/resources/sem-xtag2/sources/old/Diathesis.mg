...
declare ?Event ?Agent
{
	{?Event=active[] ; ?Agent=Subject[]; 
	 <sem>{agent(?Event.?Idx, ?Agent.?Idx)}} | betaVn[]*=[vmode = ger]
}


class  dian0Vn1active
declare ?Event ?Agent ?Patient
{
        {?Event=active[] ; ?Agent=Subject[] ; ?Patient=Object[] ; 
         <sem>{agent(?Event.?Idx, ?Agent.?Idx); patient(?Event.?Idx, ?Patient.?Idx)}}
...
declare ?Event ?Agent ?Patient
{
        ?Event=passive[] ; ?Patient=Subject[] ; ?Agent=ByAgent[] ;
        <sem>{agent(?Event.?Idx, ?Agent.?Idx); patient(?Event.?Idx, ?Patient.?Idx)}
}


class  Dian0BEn1
declare ?Event ?Agent1 ?Agent2
{
        ?Agent1=Subject[] ; 
        ?Agent2=BEComplement[] ;
        {?Event=active[] | VerbalInverted[]} ;
        <sem>{agent(?Event.?Idx, ?Agent1.?Idx); agent(?Event.?Idx, ?Agent2.?Idx)}
}
...
class  diaItVn1s2
{
        {activeCleft[] |  VerbalInvertedCleft[]}; NominalCleft[] 
}


class  diaItVpn1s2
{
        {activeCleft[] | VerbalInvertedCleft[]} ; PPCleft[]
}


class  diaItVad1s2
{
        {activeCleft[] | VerbalInvertedCleft[]} ; AdverbCleft[]
}
...
% commented out since I removed dep
%class  Dian0A1s1
%{
%        Dian0A1[] ; CanSentComplement2[amod]
%}
...
%class  Dian0N1s1
%{
%        Dian0N1[] ; CanSentComplement2[nmod]
%}
...
class  Dian0ARB1
{
        Subject[] ; verbless[] ; ARBAnchorExhaustive[]
}
