...
public class OntoModel {
	...
	private OWLClass addNewClass(String className) {
		...
		// whenever we add the axiom, we add also the individual belonging to this class
	...
	private OWLAxiom getAxiomFromString(String axiomString) {
	    ...
	    if (axiomString.startsWith("SubClassOf")||axiomString.startsWith("EquivalentClasses")) {
	    	try {  // parse only SubClassOf and Equivalent classes axioms!	
	    	...
	    	if (cls == null){
	    		...
	    		if (clsLemma != null) {
		    		...
		    		if (conceptLemmatised.contains("_+_")) {
			    		...
			    		entitiesToUri.put(conceptNameNoColon, "<http://airbus-group.installsys/component#" + conceptNameMWE + ">");
			    		...
			    		entitiesToUri.put(conceptNameNoColon, "<http://airbus-group.installsys/component#" + conceptLemmatised + ">");
	...
	public void writeOutputToFiles(String outputFolderName) throws UnsupportedEncodingException, FileNotFoundException, IOException {
		...
		str.append("\tNb. of rejected SIDP axioms that caused the ontology inconsistency = " + countAxiomsRejectedInconsistent + "\n\n");
		str.append("====Summary====\n");
	...
	public boolean checkOntologyConsistency(OWLOntology ontology) {
