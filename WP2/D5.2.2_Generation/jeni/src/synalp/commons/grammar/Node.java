...
public class Node
{
	...
	/*Changes made by Bikash -- Start */
	// Default Values
	int beforeIndex = -2;
	int index = -2;
	int afterIndex = -2;
	
	public void setWordIndices(Word inputWord){
		//System.out.println("Word = "+inputWord);
		//System.out.println("Anchor = "+anchor); 
		if (anchor!=null) {
			if (anchor.getValue().equalsIgnoreCase(inputWord.getToken())){
				beforeIndex = inputWord.getBeforeIndex();
				index = inputWord.getIndex();
				afterIndex = inputWord.getAfterIndex();
			}
		}
	}
	public int[] getIndices() {
		int[] val = {beforeIndex,index,afterIndex};
		return val;
	}
	/*Changes made by Bikash -- End */
	...
	public Node(Node node, Node parent)
	{
		...
		/*Changes made by Bikash -- Start */
		this.beforeIndex = node.beforeIndex;
		this.index = node.index;
		this.afterIndex = node.afterIndex;
		/*Changes made by Bikash -- End */
	}
